...
import { getCases, createCase, updateCase, deleteCase } from '../services/caseService.js';
import { FaUserCircle, FaEnvelope, FaPhone, FaIdCard, FaBriefcase, FaStar } from 'react-icons/fa';
...
function LawFirmDashboard() {
  ...
  useEffect(() => {
    const fetchLawyerData = async () => {
      try {
        ...
        // Fetch lawyer profile data
        const response = await fetch(`http://127.0.0.1:8000/lawyers/available_lawyers/`, {
          headers: { 
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch lawyer data. Status: ${response.status}`);
        }
        ...
        const currentLawyer = data.find(lawyer => 
          lawyer.user_details?.username === username
        );

        if (currentLawyer) {
          setLawyerProfile(currentLawyer);
          setUser(currentLawyer.user_details);
        } else {
          // If not in available lawyers, use stored data
          setUser({
            username: username,
  ...
  // Fetch prioritized cases from API
  useEffect(() => {
    const fetchPrioritizedCases = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch('http://127.0.0.1:8000/cases/prioritized_cases/', {
          headers: { 
            'Authorization': `Token ${token}`,
            'Content-Type': 'application/json'
          },
        ...
        if (response.ok) {
          const data = await response.json();
          setPrioritizedCases(data.cases || []);
        }
      } catch (error) {
        console.error("Error fetching prioritized cases:", error);
      }
    };

    fetchPrioritizedCases();
  }, []);
        ...
        <div className="d-flex align-items-center flex-wrap gap-4">
          ...
          {lawyerProfile && user ? (
            <div className="flex-grow-1">
              ...
              <p className="mb-3 opacity-75">@{user.username}</p>
              ...
              <div className="row g-3">
                ...
                <div className="col-md-6">
                  <p className="mb-2 d-flex align-items-center">
                    ...
                    <span className="badge bg-light text-dark">
                      License: {lawyerProfile.license_number}
                    </span>
                  </p>
                </div>
                ...
                <div className="col-md-6">
                  <p className="mb-2 d-flex align-items-center">
                    ...
                    <span className="badge bg-light text-dark">
                      Experience: {lawyerProfile.experience_years} years
                    </span>
                  </p>
                </div>
                ...
                <div className="col-12">
                  <p className="mb-2 d-flex align-items-center">
                    ...
                    <span className="ms-2">{lawyerProfile.rating}/5.0</span>
                    <span className="ms-3 badge bg-light text-dark">
                      {lawyerProfile.total_cases} Total Cases
                    </span>
                  </p>
                </div>
              </div>
            ...
            <div className="flex-grow-1">
              ...
              <p className="text-light opacity-75">Lawyer profile details loading...</p>
            </div>
          ) : (
            <div className="ms-4">
        ...
        {['prioritizedCases', 'updates', 'documentManagement', 'billing'].map((section) => {
          ...
          return (
            <button
              ...
              className={`btn ${selectedSection === section ? 'btn-primary' : 'btn-outline-primary'} rounded-pill px-4 py-2 shadow-sm fw-semibold`}
      ...
      {selectedSection === 'prioritizedCases' && (
        ...
        >
          ...
          {prioritizedCases.length > 0 ? (
                ...
                <tbody>
                  {prioritizedCases.map((caseData, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        <span className={`badge ${
                          caseData.urgency_score >= 80 ? 'bg-danger' :
                          caseData.urgency_score >= 60 ? 'bg-warning' :
                          'bg-success'
                        }`}>
                      ...
                      <td>
                        <span className="badge bg-info">{caseData.status}</span>
                      </td>
                      <td>
                        {caseData.days_until_deadline !== null
                          ? caseData.days_until_deadline < 0
                            ? `${Math.abs(caseData.days_until_deadline)} days overdue`
                            : `${caseData.days_until_deadline} days left`
                          : 'No deadline'}
                      </td>
                    </tr>
                  ))}
            ...
            <p className="text-muted">No prioritized cases found.</p>
          )}
        </motion.section>
      )}
      ...
      <section className="p-4 bg-white mb-4 shadow rounded">
        <div className="row">
          <div className="col-md-6">
            ...
            >
              <h4 className="fw-bold m-0">Manage Cases</h4>
              <button
